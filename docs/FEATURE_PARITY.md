...
> Integration update: the V2 release-candidate branch now mounts the Dictate, Overlay, History,
> Notes, Settings, and Vocabulary implementations described as pending in the baseline audit below.
> The baseline table is retained as historical audit evidence; current acceptance still depends on
> `QA_CHECKLIST.md` hardware testing.
...
- The release-candidate resolves and persists a separate immutable SHA for the mirror and upstream
  fallback, can reuse immutable cached revisions offline, and refuses remote-code loading when no SHA
  can be established.
- Focus capture now records the focused standard Windows child control and blocks password controls
  when detectable. Custom-rendered browser/app fields remain opaque, so manual safety testing and a
  documented limitation are still required.
